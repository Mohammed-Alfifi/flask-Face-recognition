...
app.config['MAIL_USERNAME'] = 'alfifi115@gmail.com'
app.config['MAIL_PASSWORD'] = 'tebx okao jwft qlic'
...
def send_mail(email, text):
    msg = Message('Successfully Registered', recipients=[email], sender='alfifi115@gmail.com', body=text)
    mail_.send(msg)
...
def sendResetMail(mail, otp):
    msg = Message('Reset Password', recipients=[mail], sender='alfifi115@gmail.com')
    ...
    while True:
        ...
        if success is True:
            ...
            for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
                ...
                y1, x2, y2, x1 = y1 * 4, x2 * 4, y2 * 4, x1 * 4
                # -*- coding: utf-8 -*-
                # if person is known then show id and name in green color and mark attendance if he is appearing for the first time in the day
...
def stats():
    ...
    with open("static/records.csv", 'rb') as f:
        result = chardet.detect(f.read())
    file_encoding = result['encoding']

    # استخدام ترميز الملف لقراءته باستخدام pandas
    df = pd.read_csv("static/records.csv", encoding=file_encoding)
